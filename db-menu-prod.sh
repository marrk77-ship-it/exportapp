...
case $choice in
  1)
    echo ""
    echo "📋 本番環境のユーザー一覧を取得中..."
    npm run db:users:prod
    ;;
  2)
    echo ""
    echo "📊 本番環境のCSVデータ件数を取得中..."
  ...
  3)
    ...
    npm run db:admin show-csv $login_id --remote --limit $limit
    ;;
  4)
    echo ""
    read -p "ログインID（例: client1）: " login_id
    ...
    npm run db:admin show-settings $login_id --remote
    ;;
  5)
    echo ""
    read -p "ログインID（例: client1）: " login_id
    ...
    if [ "$confirm" = "yes" ]; then
      npm run db:admin delete-csv $login_id --remote
      echo ""
      echo "✅ 削除完了"
    else
      echo ""
  ...
  6)
    ...
    echo ""
    echo "🔍 SQLクエリを実行中..."
    npm run db:admin sql "$query" --remote
    ;;
  0)
    echo ""
    echo "👋 終了します"
