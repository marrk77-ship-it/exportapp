...
- **OS社専用システム**: https://exportapp-tw.tech/os （委附表2自動生成）
...
**OS社専用システム（/os）のアカウント:**
- **OS社担当者1**: ログインID: `os_user1` / パスワード: `os2024`
- **OS社管理者**: ログインID: `os_admin` / パスワード: `os2024`
...
- **開発環境**: https://3000-iguqoisbr4aual0c9ips6-18e660f9.sandbox.novita.ai
  - **OS社専用システム**: https://3000-iguqoisbr4aual0c9ips6-18e660f9.sandbox.novita.ai/os
- **本番環境**: （デプロイ後に表示されます）
...
OS社専用の産業廃棄物管理票交付等状況報告書システムです。
CSVファイルから委附表2（新産廃税申告書）を自動生成できます。

### アクセス方法

- **開発環境**: https://3000-iguqoisbr4aual0c9ips6-18e660f9.sandbox.novita.ai/os
- **本番環境**: https://exportapp-tw.tech/os

### ログイン情報

- **ログインID**: `os_user1` または `os_admin`
- **パスワード**: `os2024`

### 主な機能

1. **CSVファイルアップロード**
   - Shift-JIS/UTF-8の自動検出
   - データプレビュー表示（最初の5件）

2. **委附表2（新産廃税）自動生成**
   - データフィルタリング:
     - ✅ 排出事業者名が「（有）オー・エス収集センター」**以外**
     - ✅ 「産業廃棄物」のみ
     - ✅ 処分方法が「最終処分」のみ
   - 月別・廃棄物種類別に自動集計
   - **元テンプレートのレイアウトを完全保持**（罫線・結合セル・フォント）
   - Excelファイルとして自動ダウンロード
...
3. **Excel生成**
   - 「委附表2（新産廃税）」ボタンをクリック
   - 自動的にExcelファイルがダウンロードされます

### 技術仕様

- **バックエンド**: Hono + Node.js child_process
- **Excel生成**: Python3 + openpyxl
- **文字エンコーディング検出**: Encoding.js
- **CSV解析**: PapaParse
- **データベース**: Cloudflare D1（os_users, os_csv_data, os_csv_uploads）

### 重要な注意事項

**開発環境のみ動作**: 現在の実装は開発環境（Sandbox）でのみ動作します。

**理由**: Cloudflare Workers環境では以下の制限があります：
- ❌ 外部プロセス実行不可（`child_process`使用不可）
- ❌ ファイルシステムアクセス不可（`fs`モジュール使用不可）
- ❌ Python実行不可

**本番環境への対応策**:
1. **外部API化**（推奨）
   - PythonスクリプトをAWS Lambda/Google Cloud Functionsなどで実行
   - CloudflareからHTTP経由で呼び出す
   - メリット: Pythonの全機能が使える、openpyxlの完全なサポート

2. **ブラウザ側処理**
   - SheetJS（xlsx）ライブラリでブラウザ側で処理
   - デメリット: 複雑な書式の完全保持が困難
